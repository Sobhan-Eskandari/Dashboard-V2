...
class RouteCollectionBuilderTest extends TestCase
{
    ...
    public function testFlushOrdering()
    {
        ...
        // 4) Add another route
        $routes->add('/admins', 'AppBundle:Admin:dashboard', 'admin_dashboard');
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        // add a "named" route
        $collectionBuilder->add('/admins', 'AppBundle:Admin:dashboard', 'admin_dashboard');
    ...
    public function testFlushSetsPrefixedWithMultipleLevels()
    {
        ...
        $routes->mount('/admins', $adminRoutes);
        ...
        $adminRoutes->import('admins.yml', '/imported');

        $collection = $routes->build();
        $this->assertEquals('/admins/dashboard', $collection->get('admin_dashboard')->getPath(), 'Routes before mounting have the prefix');
        $this->assertEquals('/admins/users', $collection->get('admin_users')->getPath(), 'Routes after mounting have the prefix');
        $this->assertEquals('/admins/blog/new', $collection->get('admin_blog_new')->getPath(), 'Sub-collections receive prefix even if mounted before parent prefix');
        $this->assertEquals('/admins/stats/sales', $collection->get('admin_stats_sales')->getPath(), 'Sub-collections receive prefix if mounted after parent prefix');
